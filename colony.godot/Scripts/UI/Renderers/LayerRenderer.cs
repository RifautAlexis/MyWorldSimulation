...
public class LayerRenderer
{
    ...
    private readonly Dictionary<TerrainType, StandardMaterial3D> _materials = new()
    {
        ...
        [TerrainType.Soil] = new StandardMaterial3D { AlbedoColor = new Color(0.8039f, 0.5216f, 0.2471f) },
        [TerrainType.Rock] = new StandardMaterial3D { AlbedoColor = new Color(0.5f, 0.5f, 0.5f) },
        [TerrainType.Water] = new StandardMaterial3D { AlbedoColor = new Color(0.0f, 0.3f, 0.9f) },
    };

    public Node3D Render(Map grid, int layer)
    {
        var root = new Node3D
        {
            Name = $"Layer_{layer}",
